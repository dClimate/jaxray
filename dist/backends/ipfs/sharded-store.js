...
export class ShardedStore {
     ...
     */
    constructor(rootCid, ipfsElements) {
        this.readOnly = true;
        this.shardDataCache = new Map();
        this.pendingShardLoads = new Map();
     ...
     */
    static async open(rootCid, ipfsElements) {
        if (!rootCid) {
    ...
    async loadRootFromCid() {
        ...
        const rootObj = dagCbor.decode(rootBytes);
    ...
    getLinearChunkIndex(chunkCoords) {
        let linearIndex = 0;
        let multiplier = 1;
    ...
    async get(key) {
        ...
        const contentBlocks = await all(stream);
    ...
    async has(key) {
        ...
        try {
            ...
            const decodedShard = await this.getOrLoadDecodedShard(shardIdx);
            if (!decodedShard)
                return false; // Shard not loaded or doesn't exist
     ...
     */
    async getOrLoadDecodedShard(shardIdx) {
        ...
        if (!shardCid) {
            ...
            if (!this.chunksPerShard)
                throw new Error("Sharding not configured.");
     ...
     */
    loadAndCacheShard(shardIdx, shardCid) {
        if (this.pendingShardLoads.has(shardIdx)) {
        ...
        const loadPromise = (async () => {
            try {
                ...
                const decodedShard = dagCbor.decode(shardBlockBytes);
