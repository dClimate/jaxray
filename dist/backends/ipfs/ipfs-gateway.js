...
export class ContentAddressedStore {
    async pin_cid(_id, _target_rpc) { }
    async unpin_cid(_id, _target_rpc) { }
    async pin_update(_oldId, _newId, _target_rpc) { }
    async pin_ls(_target_rpc) {
        return [];
    }
}
export class KuboCAS extends ContentAddressedStore {
    constructor(opts = {}) {
        super();
        const { hasher = "blake3", rpcBaseUrl = KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL, gatewayBaseUrl = KuboCAS.KUBO_DEFAULT_LOCAL_GATEWAY_BASE_URL, concurrency = 32, headers, auth = null, pinOnAdd = false, chunker = "size-1048576", maxRetries = 3, initialDelay = 1.0, backoffFactor = 2.0, fetchImpl = globalThis.fetch, } = opts;
        ...
        if (initialDelay <= 0)
            throw new Error("initial_delay must be positive");
        if (backoffFactor < 1.0)
            throw new Error("backoff_factor must be >= 1.0");
        this.fetchImpl = fetchImpl ?? (() => { throw new Error("fetch is not available"); });
        this.hasher = hasher;
    ...
    async retrying(fn) {
        ...
        while (attempt <= this.maxRetries) {
            try {
                ...
                attempt++;
                const isTimeoutOrNetwork = err?.name === "AbortError" ||
                    err?.code === "ECONNRESET" ||
                    err?.type === "system" ||
    ...
    // --------------------------------- save ------------------------------------
    async save(data, codec) {
        return this.sem.withPermit(async () => this.retrying(async () => {
    ...
    // --------------------------------- load ------------------------------------
    async load(id, offset, length, suffix) {
    ...
    // --------------------------------- pin ops ---------------------------------
    async pin_cid(cid, targetRpc = KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL) {
        ...
        await this.sem.withPermit(async () => {
            ...
            if (!res.ok)
                throw new Error(`pin/add failed: ${res.status} ${res.statusText}`);
        });
    }
    async unpin_cid(cid, targetRpc = KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL) {
        const url = `${targetRpc.replace(/\/+$/, "")}/api/v0/pin/rm?recursive=true&arg=${encodeURIComponent(String(cid))}`;
        await this.sem.withPermit(async () => {
            const res = await this.fetchImpl(url, { method: "POST", headers: this.buildHeaders() });
            if (!res.ok)
                throw new Error(`pin/rm failed: ${res.status} ${res.statusText}`);
        });
    }
    async pin_update(oldId, newId, targetRpc = KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL) {
        ...
        await this.sem.withPermit(async () => {
            ...
            if (!res.ok)
                throw new Error(`pin/update failed: ${res.status} ${res.statusText}`);
        });
    }
    async pin_ls(targetRpc = KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL) {
        const url = `${targetRpc.replace(/\/+$/, "")}/api/v0/pin/ls`;
        return this.sem.withPermit(async () => {
            const res = await this.fetchImpl(url, { method: "POST", headers: this.buildHeaders() });
            if (!res.ok)
                throw new Error(`pin/ls failed: ${res.status} ${res.statusText}`);
            const json = await res.json();
...
KuboCAS.KUBO_DEFAULT_LOCAL_GATEWAY_BASE_URL = "http://127.0.0.1:8080";
KuboCAS.KUBO_DEFAULT_LOCAL_RPC_BASE_URL = "http://127.0.0.1:5001";
KuboCAS.DAG_PB_MARKER = 0x70; // for parity with Python check
//# sourceMappingURL=ipfs-gateway.js.map