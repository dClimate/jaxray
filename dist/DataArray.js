...
export class DataArray {
    constructor(data, options = {}) {
        this._precision = 6;
        if (options.lazy) {
            if (!options.virtualShape)
                throw new Error("lazy DataArray requires virtualShape");
            ...
            for (let i = 0; i < this._dims.length; i++) {
                ...
                if (!this._coords[d])
                    this._coords[d] = Array.from({ length: this._shape[i] }, (_, j) => j);
            }
            return;
        }
        ...
        if (options.dims) {
            if (options.dims.length !== this._shape.length) {
                throw new Error(`Number of dimensions (${options.dims.length}) does not match data shape (${this._shape.length})`);
            }
        ...
        if (options.coords) {
            for (const [dim, coords] of Object.entries(options.coords)) {
                ...
                if (coords.length !== this._shape[dimIndex]) {
                    throw new Error(`Coordinate '${dim}' length (${coords.length}) does not match dimension size (${this._shape[dimIndex]})`);
                }
    ...
    async *selStream(selection, options) {
        ...
        const elementsPerSlice = this._shape.reduce((acc, size, i) => i === chunkDimIndex ? acc : acc * size, 1);
    ...
    sum(dim) {
        if (!dim) {
            ...
            return flatData.reduce((a, b) => a + b, 0);
        }
        const dimIndex = this._dims.indexOf(dim);
        if (dimIndex === -1) {
            throw new Error(`Dimension '${dim}' not found`);
        }
        const result = this._reduce(dimIndex, (acc, val) => acc + val);
    ...
    mean(dim) {
        ...
        const sumResult = this._reduce(dimIndex, (acc, val) => acc + val);
     ...
     */
    _convertCFTimeToDate(value, unit, referenceDate) {
    ...
    getBounds(options) {
        ...
        const latValues = latCoords.filter(v => typeof v === 'number');
        const lonValues = lonCoords.filter(v => typeof v === 'number');
     ...
     * ```
     */
    calculateTimeResolution(timeDim = 'time') {
    ...
    async _selLazy(selection, options) {
    ...
    _findCoordinateIndex(dim, value, options) {
     ...
     */
    _findIndexFallback(coords, value, method, tolerance) {
     ...
     */
    _findNearestIndex(coords, value, tolerance) {
        ...
        let closestIndex = 0;
        let minDiff = Math.abs(coords[0] - value);
        for (let i = 1; i < coords.length; i++) {
            const diff = Math.abs(coords[i] - value);
            if (diff < minDiff) {
                minDiff = diff;
                closestIndex = i;
            }
     ...
     */
    _findFfillIndex(coords, value, tolerance) {
        if (typeof value !== 'number' || !coords.every(c => typeof c === 'number')) {
     ...
     */
    _findBfillIndex(coords, value, tolerance) {
        if (typeof value !== 'number' || !coords.every(c => typeof c === 'number')) {
    ...
    _selectAtDimension(data, dimIndex, index) {
        if (dimIndex === 0) {
            return data[index];
        }
        ...
        return data.map((item) => this._selectAtDimension(item, dimIndex - 1, index));
    }
    _selectMultipleAtDimension(data, dimIndex, indices) {
        if (dimIndex === 0) {
        ...
        return data.map((item) => this._selectMultipleAtDimension(item, dimIndex - 1, indices));
    }
    _sliceAtDimension(data, dimIndex, start, stop) {
        if (dimIndex === 0) {
        ...
        return data.map((item) => this._sliceAtDimension(item, dimIndex - 1, start, stop));
    }
    _getCoordinateSlice(dim, start, stop) {
        const coords = this._coords[dim];
        ...
        if (dimIndex === 0) {
            ...
            if (Array.isArray(data[0])) {
                ...
                return data.reduce((acc, row) => {
                    ...
                    return reducer(acc, row);
                });
            }
            else {
                // Simple 1D reduction
                return data.reduce((acc, val) => reducer(acc, val), 0);
            }
        }
        else {
            ...
            return data.map((item) => {
                const subArray = new DataArray(item, {
                    ...
                    coords: Object.fromEntries(Object.entries(this._coords).filter(([k]) => this._dims.slice(1).includes(k)))
                });
    ...
    _divideArray(data, divisor) {
        if (!Array.isArray(data)) {
            return data / divisor;
        }
        return data.map((item) => this._divideArray(item, divisor));
    }
    /**
     * Select best dimension to chunk along for streaming
     */
    _selectChunkDimension(selection) {
