...
console.log('\n=== Example 3: Selecting data ===');
const selectedTemp = temperatures.sel({ time: 'Wed' });
console.log('Temperature on Wednesday:', selectedTemp.data);

const multiSelect = temperatures.sel({ time: ['Mon', 'Wed', 'Fri'] });
console.log('Temperatures for Mon, Wed, Fri:', multiSelect.data);

const sliceSelect = temperatures.sel({ time: { start: 'Tue', stop: 'Thu' } });
...
const multiIndex = temperatures.isel({ time: [0, 2, 4] });
...
// Example 6: Creating a Dataset
console.log('\n=== Example 6: Dataset with multiple variables ===');
...
// Example 7: Selecting from Dataset
console.log('\n=== Example 7: Selecting from Dataset ===');
const locationData = weatherData.sel({ lat: 40.0, lon: -73.5 });
...
// Example 9: Mapping over Dataset variables
console.log('\n=== Example 9: Mapping over Dataset ===');
const normalizedData = weatherData.map((dataArray, name) => {
